  ...
  return (
    <div>
      <h2 id="product-heading" data-cy="ProductHeading">
        <Translate contentKey="accountingImportExportProductsApp.product.home.title">Products</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="accountingImportExportProductsApp.product.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/product/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="accountingImportExportProductsApp.product.home.createLabel">Create new Product</Translate>
          </Link>
        </div>
      </h2>
        ...
        {productList && productList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="accountingImportExportProductsApp.product.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('costPerPiece')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('costPerPiece')} />
                </th>
                <th>
            ...
            <tbody>
              {productList.map((product, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{product.name}</td>
                  <td>
                    {product.productUnit ? <Link to={`product-unit/${product.productUnit.id}`}>{product.productUnit.name}</Link> : ''}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="accountingImportExportProductsApp.product.home.notFound">No Products found</Translate>
            </div>
          )
        )}
      </div>
